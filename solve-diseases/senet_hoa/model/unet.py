...
class TimmUnet(AbstractModel):
    def __init__(self, encoder='resnet34', in_chans=2, num_class=3, pretrained=True, channels_last=False,
                 **kwargs):
        ...
        self.seg_head =  UnetDecoderLastConv(self.decoder_filters[0], self.last_upsample_filters, num_class)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.cls_head = nn.Sequential(nn.Linear(640, 1024, bias=True), nn.Linear(1024, num_class, bias=True))
    ...
    def forward(self, x):
        ...
        cls_mask = self.cls_head(cls_feature)

        return seg_mask, cls_mask
...
class TimmUnet_v2m(AbstractModel):
    def __init__(self, encoder='resnet34', in_chans=2, num_class=3, pretrained=True, channels_last=False,
                 **kwargs):
        if not hasattr(self, 'first_layer_stride_two'):
            self.first_layer_stride_two = True
