...
class Configs(Base):
    ...
    def __init__(self,is_inference=False,sample_valid=True):
        ...
        self.model = TimmUnet_v2m(encoder="tf_efficientnetv2_m_in21k", in_chans=5, num_class=1, pretrained=not is_inference)
        ...
        train_albums = [
            ...
            A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.2, rotate_limit=90,value=80,mask_value=0,border_mode=cv2.BORDER_CONSTANT,p=0.5),
            A.RandomCrop(self.CROP_SIZE,self.CROP_SIZE,always_apply=True),
            MaskBinarize(),
            Normalize(),
        ]
        self.train_transform = A.Compose(train_albums)
        self.train_dataset = Slices3DDataset(self.TRAIN_DATA_PATHS,transforms=self.train_transform,slice=2)
        self.valid_dataset = Slices3DDataset(self.VALID_DATA_PATHS,transforms=self.test_transform,slice=2)
        if sample_valid:
            self.valid_dataset.slices = [self.valid_dataset.slices[x] for x in range(0,len(self.valid_dataset.slices),len(self.valid_dataset.slices)//300)]
